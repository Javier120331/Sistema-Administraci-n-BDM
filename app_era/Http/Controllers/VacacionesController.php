...
class VacacionesController extends Controller
{
    ...
    */
    ...
    public function getAjaxData(){
        ...
        $cantidadVacaciones = Vacacion::count();
        if($searchValue!= ""){
          if($columnIndex == 'rut_empleado' || $columnIndex == 'nombre_empleado'){
            //TODO: HACER FILTRO
            //$vacaciones= Vacacion::where('nombre_empleado','like',"%$searchValue%")
            $vacaciones= Vacacion::skip($row)->take($rowsPerPage)
                ->get()
                ->sortBy(function($vacacion){
                  return $vacacion->periodo()->first()->empleado()->first()->rut_empleado;
              });
          } else {
            $vacaciones= Vacacion::where('nombre_empleado','like',"%$searchValue%")
          ...
          //TODO: FILTRAR
          //$cantidadVacacionesFiltrados = Vacacion::where('nombre_empleado'
          // ,'like',"%$searchValue%")->count();
            $cantidadVacacionesFiltrados = Vacacion::count();          
          } else {
            if($columnIndex == 'rut_empleado' || $columnIndex == 'nombre_empleado'){
              $vacaciones= Vacacion::skip($row)->take($rowsPerPage)
                ->get()
       ...
       for($i=0; $i< count($vacaciones); ++$i){
         $vacacion = $vacaciones[$i];
         $vacacion->rut_empleado = $vacacion->periodo()->first()
            ->empleado()->first()->rut;
         $vacacion->nombre_empleado = $vacacion->periodo()->first()
            ->empleado()->first()->nombre;
         $vacacion->fecha_registro_proc = $vacacion->fecha_registro
          ->format('d/m/Y');
         $vacacion->fecha_inicio_proc = $vacacion->fecha_inicio
           ->format('d/m/Y');
         $vacacion->fecha_termino_proc = $vacacion->fecha_termino
             ->format('d/m/Y');
