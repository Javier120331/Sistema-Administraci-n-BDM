  ...
  */
  ...
  window.enviarFeriados = function(feriados, anio){
    ...
    ajax.done(function(respuesta){
      ...
      if(respuesta.resultado == true){
          $(".mensaje_feriados")
            .append("<span class='alert alert-primary'>Importación Realizada</span>");
            window.cargarFeriadosExistentes();
       } else {
         $(".mensaje_feriados")
           .append("<span class='alert alert-danger'>Ha Ocurrido un error</span>");
       }
    });
  };
  ...
  window.agregarFeriado = function(fecha, descripcion){
   ...
   ajax.done(function(respuesta){
     ...
     if(respuesta.resultado == true){
       $(".mensaje_feriados")
         .append("<span class='alert alert-primary'>Dia Festivo Agregado</span>");
        window.cargarFeriadosExistentes();
     } else {
       $(".mensaje_feriados")
         .append("<span class='alert alert-warning'>"+respuesta.mensaje+"</span>");
     }
   });
   ajax.fail(function(respuesta){
     $(".mensaje_feriados").empty();
     $(".mensaje_feriados")
       .append("<span class='alert alert-danger'>Error al procesar solicitud</span>");
   });
  }
  ...
  $("body").on("click", "#agregar_feriado_btn", function(){
     ...
     $mensajes.empty();
     if(descripcion == ""){
       $mensajes.append("<span class='alert alert-warning'>Debe ingresar una descripción</span>");
     } else {
  ...
  $("body").on("change", '.anio_select', function(){
      var fechaSeleccionada = $.fullCalendar.moment($(this).val()+'-01-01');
      $('#feriados_calendar').data().fullCalendar.view.setDate(fechaSeleccionada);
  });

  $("body").on("click", '#importar_feriados_btn', function(){
     $("#cargandoModal").modal('show').on("shown.bs.modal", function(){
       var ajax = $.ajax({
         type:'GET',
         url: window.feriadosUrl
       });
       ajax.done(function(respuesta){
         var feriados = window.obtenerFeriados(respuesta.data
            , $(".anio_select").val())
         ;
         window.enviarFeriados(feriados, $(".anio_select").val());
       });
       ajax.fail(function(respuesta){
         $(".mensaje_feriados").empty();
         $("#cargandoModal").modal("hide");
