...
class MovimientosController extends Controller
{
  ...
  public function getAjaxData(){
     ...
     if($searchValue!= ""){
      $query = MovimientoAsistenciaExport::where(function($q) use ($searchValue){
        $q->where('nombre_empleado','like',"%$searchValue%")
        ->orWhere("rut_empleado", 'like',"%$searchValue%");
      })->where('tipo_movimiento', '!=', MovimientosUtils::TIPO_LICENCIA)
        ->skip($row)->take($rowsPerPage)
        ->orderBy("fecha_inicio_movimientos","desc")
        ->orderBy($columnIndex, $columnSortOrder);
     } else {
       $query = MovimientoAsistenciaExport::where('tipo_movimiento', '!=', MovimientosUtils::TIPO_LICENCIA)
       ->skip($row)->take($rowsPerPage)
       ->orderBy("fecha_inicio_movimientos","desc")
       ->orderBy($columnIndex, $columnSortOrder);
     ...
     $cantidadMovimientosFiltrados = $query->count();
     ...
     for($i=0; $i< count($movimientos); ++$i){
       ...
       $movimiento->fecha_inicio_movimiento_proc = $movimiento
        ...
        if($movimiento->fecha_termino_movimientos == null){
          $movimiento->fecha_termino_movimiento_proc = $movimiento->fecha_inicio_movimiento_proc;
        } else {
